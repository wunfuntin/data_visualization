...
from src.utils.common_functions import (
    ...
    filter_dataframe,
    create_3d_correlation_plot,
    filter_dataframe_3d_scatter,
)

from src.utils.pdb_management_functions import (
    get_top_pdbs_from_silent,
    get_pdb_sequences,
)
...
@callback(Output("dataframe-json", "children"), [Input("upload-csv", "contents")])
...
@callback(Output("dropdown", "options"), [Input("dataframe-json", "children")])
def update_dropdown_options(csv_json):
    if csv_json is not None:
        df = pd.read_json(io.StringIO(csv_json), orient="split")
        return [
            {"label": col, "value": col} for col in df.columns if col != "description"
        ]
    else:
...
def update_axis_column_options(csv_json):
    if csv_json:
        ...
        return [
            [] for _ in range(7)
        ]  # Return empty options for all dropdowns if no data
        ...
        fig = go.Figure(
            data=[
                go.Scatter(
                    ...
                    text=df.apply(
                        lambda row: generate_hover_text(row, df.columns), axis=1
                    ),
                    hoverinfo="text",
                )
            ]
    ...
    if click_data is not None:
        ...
        for entry in data_entries:
            if entry.startswith("design_name:"):
                design_name = entry.split(": ")[
                    1
                ].strip()  # Split by ': ' and strip whitespace
                break
...
@callback(
    ...
    [Input("dataframe-json", "children"), Input("scatter-plot", "hoverData")],
)
...
def update_scatter3d_plot(
    csv_json, x_col, y_col, z_col, x_min, x_max, y_min, y_max, z_min, z_max
):
    if (
        csv_json is not None
        and x_col is not None
        and y_col is not None
        and z_col is not None
    ):
        df = pd.read_json(io.StringIO(csv_json), orient="split")
        filtered_df = filter_dataframe_3d_scatter(
            df, x_col, y_col, z_col, x_min, x_max, y_min, y_max, z_min, z_max
        )
...
@callback(
    [Output("common-data-table", "data"), Output("filtered-data", "children")],
    [
        Input("dataframe-json", "children"),
...
def update_common_data_table(
    csv_json,
    selected_correlation_1,
    selected_correlation_2,
    x_min_1,
    x_max_1,
    y_min_1,
    y_max_1,
    ...
    try:
        if csv_json:
            ...
            filtered_df_1 = filter_dataframe(
                df, x_col_1, y_col_1, x_min_1, x_max_1, y_min_1, y_max_1
            )
            filtered_df_2 = filter_dataframe(
                df, x_col_2, y_col_2, x_min_2, x_max_2, y_min_2, y_max_2
            )
            filtered_df_3 = filter_dataframe_3d_scatter(
                df,
                x_col_1,
                y_col_1,
                z_col_1,
...
def download_csv(n_clicks, filtered_json):
    if n_clicks is not None and filtered_json is not None:
        ...
        return dcc.send_data_frame(
            filtered_df.to_csv, filename="common_data_points.csv"
        )
    else:
...
@callback(
    [
        ...
        Output("parent-pdb-directory", "data"),
    ],
...
def get_pdb_files(pdb_list):
    if pdb_list is not None:
        ...
        sequences = get_pdb_sequences(directory)
        return pdb_file_list, pdb_path_list, parent_directory
